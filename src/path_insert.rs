...
/// expansion rules. The resolved inode is delegated directly to [`insert_file_blocks_journaled`],
/// keeping allocation, inode growth, new data-block publication, WAL recovery, and checkpoint
/// semantics centralized in the inode-ID-based primitive.
///
...
/// # Errors
///
/// Propagates pathname lookup errors and all [`insert_file_blocks_journaled`] validation or durable
/// I/O errors, including a resolved non-file inode, an empty insertion, an insertion index beyond
/// the current logical block count, insufficient free space, or insufficient journal capacity.
