...
/// Inserts multiple complete logical blocks into an existing regular file as one contiguous run.
///
...
/// insertion. Format v5 still persists an explicit block vector, so contiguity is an allocation-time
...
/// the current logical block count, a block-count conversion failure, or lack of a sufficiently large
/// contiguous free run. Journal-capacity, encoding, I/O, recovery, checkpoint, and flush failures are
/// propagated. A failure may occur after commit is durable; callers must recover and checkpoint before
/// interpreting home state.
pub fn insert_file_blocks_contiguous_journaled(
    ...
    if data_blocks.is_empty() {
        return Err(io::Error::new(
            ...
            "contiguous multi-block insert requires at least one data block",
        ));
    }
    let block_count = u64::try_from(data_blocks.len()).map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            "contiguous multi-block insert exceeds the block address space",
        )
    })?;
    ...
    capture.collect_changed_range(
        ...
        "contiguous multi-block insert image did not render every allocation metadata block",
        &mut changed,
    )?;
    capture.collect_changed_range(
        device,
        superblock.inode_range(),
        "contiguous multi-block insert image did not render every inode metadata block",
        &mut changed,
    )?;
    capture.ensure_empty(
        "contiguous multi-block insert image rendered outside allocation and inode regions",
    )?;
    ...
    if report.committed_transactions != 1 || report.home_writes != changed.len() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "contiguous multi-block insert recovery report is inconsistent",
        ));
    }
