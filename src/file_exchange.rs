...
pub fn exchange_file_block_ranges_journaled(
    ...
    if left_inode_id == right_inode_id || block_count == 0 {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, "block-range exchange requires distinct files and a non-empty range"));
    }
    let allocator = load_allocator(device, superblock)?;
    let mut inodes = load_inode_table(device, superblock)?;
    let left_pos = inodes.iter().position(|inode| inode.id == left_inode_id)
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "left inode is missing"))?;
    let right_pos = inodes.iter().position(|inode| inode.id == right_inode_id)
    ...
    if inodes[left_pos].kind != InodeKind::File || inodes[right_pos].kind != InodeKind::File {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, "block-range exchange endpoints must be regular files"));
    }
    let left_end = left_index.checked_add(block_count).ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "left range overflows"))?;
    let right_end = right_index.checked_add(block_count).ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "right range overflows"))?;
    if left_end > inodes[left_pos].blocks.len() || right_end > inodes[right_pos].blocks.len() {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, "block-range exchange is beyond file end"));
    }
    let mut seen = HashSet::new();
    for block in inodes[left_pos].blocks.iter().chain(&inodes[right_pos].blocks).copied() {
        if !seen.insert(block) {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "exchange endpoints contain duplicate physical-block references"));
        }
        if !allocator.is_owned(block).map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))? {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "exchange endpoint references a block that is not allocator-owned"));
        }
    }
    ...
    capture.collect_changed_range(device, superblock.inode_range(), "block-range exchange did not render every inode metadata block", &mut changed)?;
    ...
    if report.committed_transactions != 1 || report.home_writes != changed.len() {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "block-range exchange recovery report is inconsistent"));
    }
    Ok(report)
}
