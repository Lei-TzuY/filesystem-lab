...
/// allocated for every cloned image, and allocator ownership, destination inode references, and the
/// new data homes are published through one WAL transaction. The source mapping and source data are
/// unchanged. Source and destination may name the same inode; in that case the operation still uses
/// snapshot semantics.
///
...
/// existing logical block list, or insufficient free blocks. Returns `InvalidData` when allocator
/// ownership disagrees with a source inode reference. Journal-capacity, encoding, recovery,
/// checkpoint, and block-device I/O failures are propagated.
pub fn clone_file_blocks_append_journaled(
    ...
    if block_count == 0 {
        return Err(io::Error::new(
            ...
            "clone append requires at least one source block",
        ));
    }
    ...
    capture.collect_changed_range(
        ...
        "clone append image did not render every allocation metadata block",
        &mut changed,
    )?;
    capture.collect_changed_range(
        device,
        superblock.inode_range(),
        "clone append image did not render every inode metadata block",
        &mut changed,
    )?;
    capture.ensure_empty("clone append image rendered outside allocation and inode regions")?;
    ...
    if report.committed_transactions != 1 || report.home_writes != changed.len() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "clone append recovery report is inconsistent",
        ));
    }
