...
pub fn read_file_block(device: &mut impl BlockDevice, superblock: &Superblock, inode_id: u64, file_block_index: usize) -> io::Result<[u8; BLOCK_SIZE]> {
...
pub fn write_file_block_journaled(device: &mut impl BlockDevice, superblock: &Superblock, inode_id: u64, file_block_index: usize, data: [u8; BLOCK_SIZE]) -> io::Result<RecoveryReport> {
    let block = resolve_owned_file_block(device, superblock, inode_id, file_block_index)?;
...
fn journal_block_image(device: &mut impl BlockDevice, superblock: &Superblock, block: u64, data: [u8; BLOCK_SIZE]) -> io::Result<RecoveryReport> {
    let mut current = [0_u8; BLOCK_SIZE];
    device.read_block(block, &mut current)?;
    if current == data { return Ok(RecoveryReport::default()); }
    ...
    if report.committed_transactions != 1 || report.home_writes != 1 {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "file-data transaction recovery report is inconsistent"));
    }
    Ok(report)
}

pub fn append_file_block_journaled(device: &mut impl BlockDevice, superblock: &Superblock, inode_id: u64, data: [u8; BLOCK_SIZE]) -> io::Result<(u64, RecoveryReport)> {
    let mut allocator = load_allocator(device, superblock)?;
    let mut inodes = load_inode_table(device, superblock)?;
    let inode = inodes.iter_mut().find(|inode| inode.id == inode_id).ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "file-data target inode is missing"))?;
    if inode.kind != InodeKind::File { return Err(io::Error::new(io::ErrorKind::InvalidInput, "file-data target must be a regular file")); }
    ...
    store_journal_image(device, *superblock, log.entries())?;
    let report = recover_journal_and_checkpoint(device, *superblock)?;
    if report.committed_transactions != 1 || report.home_writes != changed.len() { return Err(io::Error::new(io::ErrorKind::InvalidData, "file append recovery report is inconsistent")); }
    Ok((block, report))
}

fn resolve_owned_file_block(device: &mut impl BlockDevice, superblock: &Superblock, inode_id: u64, file_block_index: usize) -> io::Result<u64> {
    let inodes = load_inode_table(device, superblock)?;
    let inode = inodes.iter().find(|inode| inode.id == inode_id).ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "file-data target inode is missing"))?;
    if inode.kind != InodeKind::File { return Err(io::Error::new(io::ErrorKind::InvalidInput, "file-data target must be a regular file")); }
    let block = *inode.blocks.get(file_block_index).ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "file-data logical block index is out of range"))?;
    ...
    if !owned { return Err(io::Error::new(io::ErrorKind::InvalidData, "file-data inode references an unowned block")); }
    Ok(block)
}
