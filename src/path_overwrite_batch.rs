...
/// expansion rules. The resolved inode is delegated directly to [`write_file_blocks_journaled`], so
/// validation, allocator-ownership checks, WAL publication, recovery, and checkpoint semantics stay
/// centralized in the existing inode-ID primitive.
///
...
/// # Errors
///
/// Propagates pathname lookup errors and all [`write_file_blocks_journaled`] validation or durable I/O
/// errors, including non-file targets, empty batches, duplicate or out-of-range logical indices,
/// allocator ownership disagreement, and insufficient journal capacity.
