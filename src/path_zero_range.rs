...
/// expansion rules. The resolved inode is delegated directly to [`zero_file_range_journaled`], so
/// existing-range validation, WAL publication, crash recovery, and checkpoint semantics remain
/// centralized in the inode-ID-based primitive.
///
...
///
/// # Errors
/// Propagates pathname lookup errors and all [`zero_file_range_journaled`] validation or durable I/O
/// errors, including a resolved non-file inode, an empty range, an invalid offset, or a range that
/// extends beyond existing logical blocks.
