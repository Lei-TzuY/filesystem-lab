...
`filesystem-lab` exposes a read-only consistency checker through `fsck::check_device`. Its scope deliberately matches the durable layers that exist today: the version-6 superblock, persistent allocation image, persistent inode table, persistent directory table, and bounded persistent journal region.
...
`fsck_repair::repair_orphaned_allocations_journaled` provides one bounded repair policy for allocation leaks. `fsck_repair::repair_unreachable_inodes_journaled` provides a separate namespace-orphan policy: after older WAL recovery, it tolerates only root-reachability failure while keeping allocator ownership, payload, endpoint, cycle, and journal checks strict. It reattaches only the top-level roots of unreachable acyclic components beneath inode 1 using deterministic collision-safe `.fsck-orphan-<inode>` names, preserves existing subtree structure and data, publishes one bounded directory-table WAL transaction, checkpoints it, and requires strict fsck to pass. Neither repair path is a best-effort general corruption mutator.
...
`check_device` remains intentionally read-only: it performs no block writes and crosses no flush boundary. Orphan-allocation and unreachable-namespace repair APIs are separate, explicitly journaled mutation paths so diagnosis and repair policy remain mechanically distinct.

## Future extension

Future extensions can add link-count validation, a stronger single-parent directory policy if desired, and additional narrowly scoped repair policies. New repair families should remain fail-closed and independently crash-tested rather than turning fsck into a general best-effort mutator.
