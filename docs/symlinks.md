...
`resolve_path_without_following_final_symlink` uses the same traversal contract for intermediate components but returns the final namespace inode without expanding it when that inode is a symlink. This makes the symlink inode itself addressable even when its opaque target is dangling. `read_symlink_at_path` first recovers and checkpoints any older committed WAL, then runs that no-follow-final lookup and validates the final `SYM1` payload through `read_symlink`, providing bounded pathname-level `readlink` semantics against recovered durable namespace state. Intermediate symlinks are still expanded and count toward the same 40-link limit; the final symlink is not expanded merely to identify or read it.

The lookup contract deliberately rejects empty components, trailing slashes on non-root paths, `.` and `..` rather than implementing normalization semantics. The low-level pathname lookup helpers remain read-only. Pathname `readlink` does not create a new WAL transaction or change filesystem format v5, but its first call after a crash may replay and checkpoint an older committed transaction before resolving the requested symlink. The dedicated recovery-boundary regression enumerates committed symlink-creation crash states and requires `read_symlink_at_path` to recover the namespace before no-follow resolution, preserve allocator/inode accounting and unique physical ownership, leave fsck clean, clear the journal, and make a second recovery a no-op.
