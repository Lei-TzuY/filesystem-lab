...
`clone_file_blocks_splice_at_path_journaled` exposes the same primitive through bounded absolute-path resolution. Intermediate and final symbolic links are followed for both endpoints before the resolved inode IDs are passed to the inode-level operation. The pathname surface adds no second durability mechanism or on-disk state.
...
The pathname crash matrix additionally requires source data and namespace state to remain unchanged across every interruption while allocator and inode images recover to exactly the old or complete new state.
