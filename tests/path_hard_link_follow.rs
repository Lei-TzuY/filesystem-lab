...
use filesystem_lab::path_hard_link::hard_link_following_source_at_path_journaled;
...
fn follows_final_source_symlink_and_links_resolved_regular_file() {
    ...
    assert_ne!(
        resolve_path_without_following_final_symlink(&mut device, &superblock, "/followed")
            .unwrap(),
        symlink_inode
    );
    assert_eq!(
        resolve_path_following_symlinks(&mut device, &superblock, "/followed").unwrap(),
        3
    );
    assert_eq!(
...
#[test]
fn rejects_final_source_symlink_that_resolves_to_directory_before_publication() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();
    let inodes_before = load_inode_table(&mut device, &superblock).unwrap();
...
fn every_following_dispatch_crash_point_recovers_old_or_complete_new_state() {
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            ...
            .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt followed-source hard-link dispatch"
        );
        device.reboot();
        ...
        if recovery.committed_transactions == 0 {
            ...
            assert_eq!(
                resolve_path_following_symlinks(&mut device, &superblock, "/linked").unwrap(),
                3
            );
        }
