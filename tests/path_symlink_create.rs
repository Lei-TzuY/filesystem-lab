...
#[test]
fn creates_symlinks_at_direct_and_symlinked_parent_paths() {
    ...
    assert_eq!(
        read_symlink_at_path(&mut device, &superblock, "/direct").unwrap(),
        "/dir"
    );
    assert_eq!(
        read_symlink_at_path(&mut device, &superblock, "/dir/nested").unwrap(),
        "../direct"
    );
    assert_eq!(
        read_symlink_at_path(&mut device, &superblock, "/dir/via_alias").unwrap(),
        TARGET
    );
...
#[test]
fn rejects_invalid_destination_forms_and_collisions_before_publication() {
    let (mut device, superblock) = setup();
    ...
    for path in ["relative", "/", "/dir/"] {
        assert_eq!(
            create_symlink_at_path_journaled(&mut device, &superblock, path, TARGET)
                .unwrap_err()
                .kind(),
    ...
    assert_eq!(
        create_symlink_at_path_journaled(&mut device, &superblock, "/missing/link", TARGET)
            .unwrap_err()
            .kind(),
        io::ErrorKind::NotFound
    );
    assert_eq!(
        create_symlink_at_path_journaled(&mut device, &superblock, "/dir", TARGET)
            .unwrap_err()
            .kind(),
        io::ErrorKind::InvalidInput
    );
...
#[test]
fn every_pathname_symlink_create_crash_point_recovers_old_or_complete_new_state() {
    let (mut probe, superblock) = setup();
    probe.arm(None);
    create_symlink_at_path_journaled(&mut probe, &superblock, "/dir_alias/link", TARGET).unwrap();
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            create_symlink_at_path_journaled(&mut device, &superblock, "/dir_alias/link", TARGET)
                .unwrap_err()
                .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt pathname symlink creation"
        );
        device.reboot();
