...
fn empty_or_out_of_bounds_range_is_rejected_before_publication() {
    let (mut device, superblock, blocks) = setup();
    let empty = collapse_file_block_range_journaled(&mut device, &superblock, 2, 1, 0)
        .unwrap_err();
    assert_eq!(empty.kind(), io::ErrorKind::InvalidInput);
    let outside = collapse_file_block_range_journaled(&mut device, &superblock, 2, 4, 2)
        .unwrap_err();
...
fn allocator_disagreement_anywhere_in_range_is_rejected_before_publication() {
    ...
    let error = collapse_file_block_range_journaled(&mut device, &superblock, 2, 2, 2)
        .unwrap_err();
