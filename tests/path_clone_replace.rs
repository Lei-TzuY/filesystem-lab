...
use filesystem_lab::path_clone_replace::{
    clone_file_blocks_replace_at_path_journaled, PathCloneReplaceRange,
};
...
fn source(path: &str, start: usize, block_count: usize) -> PathCloneReplaceRange<'_> {
...
#[test]
fn replaces_cloned_blocks_through_intermediate_and_final_symlink_paths() {
...
#[test]
fn rejects_invalid_path_clone_replace_without_publication() {
    let (mut device, superblock) = setup();
...
#[test]
fn every_path_clone_replace_crash_point_recovers_old_or_complete_new_state() {
    ...
    for crash_at in 0..operations {
        ...
        assert!(
            old || new,
            "crash point {crash_at} recovered a mixed clone-replace state"
        );
        assert_eq!(
            load_directory_table(&mut device, &superblock).unwrap(),
            directory_before
