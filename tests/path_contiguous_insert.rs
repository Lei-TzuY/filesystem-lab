...
use filesystem_lab::path_contiguous_insert::insert_file_blocks_contiguous_at_path_journaled;
...
#[test]
fn inserts_one_contiguous_run_at_an_interior_boundary_through_a_final_symlink() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();

    let (blocks, _) = insert_file_blocks_contiguous_at_path_journaled(
        &mut device,
        &superblock,
        "/file_alias",
        1,
...
#[test]
fn rejects_invalid_contiguous_insert_without_persistent_change() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();
...
#[test]
fn every_contiguous_insert_crash_point_recovers_old_or_complete_new_state() {
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            ...
            .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt contiguous insert"
        );
        device.reboot();
