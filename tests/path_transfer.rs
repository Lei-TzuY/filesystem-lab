...
fn setup() -> (CrashDevice, Superblock) {
    ...
    store_inode_table(&mut device, &superblock, &[
        ...
        inode(3, InodeKind::File), inode(4, InodeKind::File),
    ]).unwrap();
    store_directory_table(&mut device, &superblock, &[
        entry(1, 2, "dir"), entry(2, 3, "source"), entry(2, 4, "destination"),
    ]).unwrap();
    ...
    append_file_blocks_at_path_journaled(&mut device, &superblock, "/dir/source", &[
        [0x11; BLOCK_SIZE], [0x22; BLOCK_SIZE], [0x33; BLOCK_SIZE],
    ]).unwrap();
    append_file_blocks_at_path_journaled(&mut device, &superblock, "/dir/destination", &[
...
fn transfer_middle(device: &mut CrashDevice, superblock: &Superblock) -> io::Result<RecoveryReport> {
    transfer_file_block_range_at_path_journaled(
        device, superblock, endpoint("/source_alias", 1), 1,
...
fn transfers_block_range_through_direct_and_symlink_paths() {
    ...
    let (moved, _) = transfer_file_block_range_at_path_journaled(
        &mut device, &superblock, endpoint("/dir_alias/source", 1), 1,
        endpoint("/destination_alias", 1),
    ).unwrap();
    assert_eq!(moved.len(), 1);
    assert_eq!(read_file_range_at_path(&mut device, &superblock, "/dir/source", 1, 0, 1).unwrap(), vec![0x33]);
    assert_eq!(read_file_range_at_path(&mut device, &superblock, "/dir/destination", 1, 0, 1).unwrap(), vec![0x22]);
    check_device(&mut device).unwrap();
}
...
fn rejects_invalid_path_transfer_without_publication() {
    ...
    for result in [
        transfer_file_block_range_at_path_journaled(&mut device, &superblock, endpoint("/dir", 0), 1, endpoint("/dir/destination", 0)),
        transfer_file_block_range_at_path_journaled(&mut device, &superblock, endpoint("/dangling", 0), 1, endpoint("/dir/destination", 0)),
        transfer_file_block_range_at_path_journaled(&mut device, &superblock, endpoint("/dir/source", 0), 1, endpoint("/source_alias", 0)),
        transfer_file_block_range_at_path_journaled(&mut device, &superblock, endpoint("/dir/source", 0), 0, endpoint("/dir/destination", 0)),
        transfer_file_block_range_at_path_journaled(&mut device, &superblock, endpoint("/dir/source", 3), 1, endpoint("/dir/destination", 0)),
    ] {
...
fn every_pathname_transfer_crash_point_recovers_old_or_complete_new_state() {
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(transfer_middle(&mut device, &superblock).unwrap_err().kind(), io::ErrorKind::Other);
        device.reboot();
