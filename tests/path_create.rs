...
#[test]
fn creates_zero_block_files_at_direct_and_symlinked_parent_paths() {
    ...
    for (path, inode_id) in [
        ("/root_file", root_file),
        ("/dir/nested", nested_file),
        ("/dir/via_alias", alias_file),
    ] {
...
fn rejects_invalid_destinations_collisions_and_non_directory_parent_before_publication() {
    ...
    for path in ["relative", "/", "/dir/"] {
        assert_eq!(
            create_empty_file_at_path_journaled(&mut device, &superblock, path)
                .unwrap_err()
                .kind(),
    ...
    assert_eq!(
        create_empty_file_at_path_journaled(&mut device, &superblock, "/missing/child")
            .unwrap_err()
            .kind(),
        io::ErrorKind::NotFound
    );
    assert_eq!(
        create_empty_file_at_path_journaled(&mut device, &superblock, "/dir")
            .unwrap_err()
            .kind(),
        io::ErrorKind::InvalidInput
    );
    assert_eq!(
        create_empty_file_at_path_journaled(&mut device, &superblock, "/fileparent/child")
            .unwrap_err()
            .kind(),
        io::ErrorKind::InvalidInput
    );
...
#[test]
fn every_pathname_empty_file_create_crash_point_recovers_old_or_complete_new_state() {
    let (mut probe, superblock) = setup();
    probe.arm(None);
    create_empty_file_at_path_journaled(&mut probe, &superblock, "/dir_alias/new_file").unwrap();
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            create_empty_file_at_path_journaled(&mut device, &superblock, "/dir_alias/new_file")
                .unwrap_err()
                .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt pathname empty-file creation"
        );
        device.reboot();
        ...
        if recovery.committed_transactions == 0 {
            ...
            let metadata = metadata_at_path(&mut device, &superblock, "/dir/new_file").unwrap();
            assert_eq!(metadata.kind, InodeKind::File);
            assert_eq!(metadata.logical_blocks, 0);
            assert_eq!(metadata.namespace_references, 1);
        }
