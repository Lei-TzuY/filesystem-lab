...
use filesystem_lab::path_contiguous_append::append_zeroed_blocks_contiguous_at_path_journaled;
...
#[test]
fn appends_zeroed_blocks_as_one_contiguous_run_through_final_symlink() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();

    let (blocks, _) = append_zeroed_blocks_contiguous_at_path_journaled(
        &mut device,
        &superblock,
        "/file_alias",
        ZERO_BLOCKS,
    ...
    assert_eq!(blocks.len(), usize::try_from(ZERO_BLOCKS).unwrap());
    ...
    for logical_block in 0..usize::try_from(ZERO_BLOCKS).unwrap() {
        assert_eq!(
...
#[test]
fn rejects_zero_block_count_without_persistent_change() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();
    ...
    assert_eq!(
        append_zeroed_blocks_contiguous_at_path_journaled(
            &mut device,
            &superblock,
            "/dir/file",
            0,
...
#[test]
fn every_zero_append_crash_point_recovers_old_or_complete_new_state() {
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            ...
            .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt contiguous zero append"
        );
        device.reboot();
