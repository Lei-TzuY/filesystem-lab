...
use filesystem_lab::path_contiguous_append::append_file_blocks_contiguous_at_path_journaled;
...
#[test]
fn appends_one_contiguous_run_through_a_final_symlink() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();

    let (blocks, _) = append_file_blocks_contiguous_at_path_journaled(
        &mut device,
        &superblock,
        "/file_alias",
        &APPEND,
    ...
    assert_eq!(blocks.len(), APPEND.len());
    assert!(blocks.windows(2).all(|pair| pair[1] == pair[0] + 1));
    assert_eq!(
        load_allocator(&mut device, &superblock)
            .unwrap()
            .allocated_blocks(),
        allocator_before.allocated_blocks() + u64::try_from(APPEND.len()).unwrap()
    );
    for (index, image) in APPEND.iter().enumerate() {
        assert_eq!(
            read_file_range_at_path(&mut device, &superblock, "/dir/file", index, 0, BLOCK_SIZE)
                .unwrap(),
            image
        );
    }
...
#[test]
fn rejects_empty_contiguous_append_without_persistent_change() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();
    let inodes_before = load_inode_table(&mut device, &superblock).unwrap();

    assert_eq!(
        append_file_blocks_contiguous_at_path_journaled(
            &mut device,
            &superblock,
            "/dir/file",
            &[],
...
#[test]
fn every_contiguous_append_crash_point_recovers_old_or_complete_new_state() {
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            ...
            .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt contiguous append"
        );
        device.reboot();
        ...
        if file_after.blocks == file_before.blocks {
            ...
            for (offset, image) in APPEND.iter().enumerate() {
                assert_eq!(
                    read_file_range_at_path(
                        &mut device,
                        &superblock,
                        "/dir/file",
                        file_before.blocks.len() + offset,
                        0,
                        BLOCK_SIZE,
                    )
                    .unwrap(),
