...
#[test]
fn transfer_recovers_committed_source_symlink_before_resolving_it() {
    ...
    for crash_at in 0..operations {
        ...
        let (moved_blocks, _) = transfer_file_block_range_at_path_journaled(
            &mut device,
            &superblock,
            endpoint("/src_alias", 1),
            1,
            endpoint("/dst", 1),
        )
        .unwrap();
        ...
        assert_eq!(
            read_file_range_at_path(&mut device, &superblock, "/src", 0, 0, 1).unwrap(),
            vec![0x11]
        );
        assert_eq!(
            read_file_range_at_path(&mut device, &superblock, "/src", 1, 0, 1).unwrap(),
            vec![0x33]
        );
        assert_eq!(
        ...
        assert!(allocator_after.is_owned(moved_blocks[0]).unwrap());
