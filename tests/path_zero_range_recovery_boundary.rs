...
#[test]
fn zero_range_recovers_committed_final_symlink_before_resolving_it() {
    let (mut probe, superblock) = setup_without_alias();
    probe.arm(None);
    ...
    for crash_at in 0..operations {
        let (mut device, superblock) = setup_without_alias();
        ...
        assert_eq!(
            read_file_range_at_path(&mut device, &superblock, "/file", 0, BLOCK_SIZE - 2, 4)
                .unwrap(),
            vec![0, 0, 0, 0]
        );
