...
fn assert_created_file(
    ...
    device.read_block(block, &mut actual).unwrap();
    assert_eq!(&actual, expected);
    block
}

#[test]
fn creates_initialized_one_block_files_at_direct_and_symlinked_parent_paths() {
    let (mut device, superblock) = setup();
    let first = [0x5a_u8; BLOCK_SIZE];
    let second = [0xa5_u8; BLOCK_SIZE];

    create_one_block_file_at_path_journaled(&mut device, &superblock, "/dir/direct", &first)
        .unwrap();
    ...
    let first_block = assert_created_file(&mut device, &superblock, "/dir/direct", &first);
    let second_block = assert_created_file(&mut device, &superblock, "/dir/via_alias", &second);
...
#[test]
fn rejects_invalid_destinations_without_allocating_or_publishing_namespace() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();
    let inodes_before = load_inode_table(&mut device, &superblock).unwrap();
    let entries_before = load_directory_table(&mut device, &superblock).unwrap();
    let data = [0x33_u8; BLOCK_SIZE];

    for path in ["relative", "/", "/dir/", "/dir"] {
        assert_eq!(
            create_one_block_file_at_path_journaled(&mut device, &superblock, path, &data)
                .unwrap_err()
                .kind(),
    ...
    assert_eq!(
        create_one_block_file_at_path_journaled(&mut device, &superblock, "/missing/child", &data)
            .unwrap_err()
            .kind(),
...
#[test]
fn every_one_block_create_crash_point_recovers_old_or_complete_new_file() {
    let data = [0xc3_u8; BLOCK_SIZE];
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            ...
            .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt pathname one-block creation"
        );
        device.reboot();
        ...
        if recovery.committed_transactions == 0 {
            ...
            assert_eq!(load_inode_table(&mut device, &superblock).unwrap().len(), inodes_before.len() + 1);
            assert_eq!(load_directory_table(&mut device, &superblock).unwrap().len(), entries_before.len() + 1);
            assert_created_file(&mut device, &superblock, "/dir/seeded", &data);
        }
