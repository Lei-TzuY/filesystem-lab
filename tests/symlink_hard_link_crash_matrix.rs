...
#[test]
fn every_symlink_hard_link_crash_point_is_old_or_recoverable_new_namespace() {
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            hard_link_symlink_journaled(&mut device, &superblock, 1, "alias", symlink)
                .unwrap_err()
                .kind(),
            io::ErrorKind::Other
        );
...
#[test]
fn symlink_hard_link_rejects_wrong_kind_and_existing_destination() {
    let (mut device, superblock, symlink) = setup();
    assert_eq!(
        hard_link_symlink_journaled(&mut device, &superblock, 1, "bad", 1)
            .unwrap_err()
            .kind(),
        io::ErrorKind::InvalidInput
    );
    assert_eq!(
        hard_link_symlink_journaled(&mut device, &superblock, 1, "original", symlink)
            .unwrap_err()
            .kind(),
        io::ErrorKind::InvalidInput
    );
