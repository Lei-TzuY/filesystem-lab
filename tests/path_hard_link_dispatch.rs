...
use filesystem_lab::path_hard_link::hard_link_at_path_journaled;
...
#[test]
fn dispatches_regular_file_and_final_symlink_without_following_the_symlink() {
    ...
    assert_eq!(
        resolve_path_following_symlinks(&mut device, &superblock, "/dir/file_link").unwrap(),
        3
    );
    assert_eq!(
        resolve_path_without_following_final_symlink(
...
#[test]
fn rejects_directory_source_and_malformed_destinations_before_publication() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();
    let inodes_before = load_inode_table(&mut device, &superblock).unwrap();
    ...
    for (source, destination) in [
        ...
        assert_eq!(
            hard_link_at_path_journaled(&mut device, &superblock, source, destination)
                .unwrap_err()
                .kind(),
...
#[test]
fn every_regular_file_dispatch_crash_point_recovers_old_or_complete_new_state() {
    let (mut probe, superblock) = setup();
    probe.arm(None);
    hard_link_at_path_journaled(&mut probe, &superblock, "/file", "/dir_alias/linked").unwrap();
...
#[test]
fn every_final_symlink_dispatch_crash_point_recovers_old_or_complete_new_state() {
    let (mut probe, superblock) = setup();
    probe.arm(None);
    hard_link_at_path_journaled(
        &mut probe,
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            ...
            .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt final-symlink hard-link dispatch"
        );
        device.reboot();
        let recovery = recover_journal_and_checkpoint(&mut device, superblock).unwrap();
