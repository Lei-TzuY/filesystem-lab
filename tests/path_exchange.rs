...
#[test]
fn exchanges_block_ranges_through_direct_and_symlink_paths() {
    let (mut device, superblock) = setup();
    exchange_file_block_ranges_at_path_journaled(
        &mut device,
        &superblock,
        range("/dir_alias/left", 0),
        range("/right_alias", 0),
        1,
    )
    .unwrap();
    ...
    assert_eq!(
        read_file_range_at_path(&mut device, &superblock, "/dir/left", 0, 0, 1).unwrap(),
        vec![0x33]
    );
    assert_eq!(
...
#[test]
fn rejects_invalid_path_exchange_without_publication() {
    let (mut device, superblock) = setup();
...
fn every_pathname_exchange_crash_point_recovers_old_or_complete_new_state() {
    ...
    probe.arm(None);
    exchange_second_blocks(&mut probe, &superblock).unwrap();
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            exchange_second_blocks(&mut device, &superblock)
                .unwrap_err()
                .kind(),
            io::ErrorKind::Other
        );
