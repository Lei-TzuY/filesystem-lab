...
use filesystem_lab::file_clone_replace::clone_file_blocks_replace_journaled;
...
#[test]
fn clone_replacement_allocates_fresh_blocks_and_releases_displaced_blocks() {
    let (mut device, superblock, source_blocks, destination_blocks, expected_new) = setup();
    let (new_blocks, displaced, report) =
        clone_file_blocks_replace_journaled(&mut device, &superblock, 2, 0, 2, 3, 1).unwrap();
...
#[test]
fn clone_replacement_rejects_same_inode() {
    let (mut device, superblock, _, _, _) = setup();
    assert_eq!(
        clone_file_blocks_replace_journaled(&mut device, &superblock, 2, 0, 1, 2, 1)
            .unwrap_err()
            .kind(),
...
#[test]
fn every_clone_replacement_crash_point_is_old_or_recoverable_new_state() {
    let (mut probe, superblock, _, _, _) = setup();
    probe.arm(None);
    let (_, _, report) =
        clone_file_blocks_replace_journaled(&mut probe, &superblock, 2, 0, 2, 3, 1).unwrap();
    assert_eq!(report.home_writes, 4);
    ...
    for crash_at in 0..mutation_operations {
        ...
        assert_eq!(
            clone_file_blocks_replace_journaled(&mut device, &superblock, 2, 0, 2, 3, 1)
                .unwrap_err()
                .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt clone replacement"
        );
        device.reboot();

        let allocator = load_allocator(&mut device, &superblock).unwrap();
        let raw_new_owned: Vec<_> = expected_new
            .iter()
            .map(|block| allocator.is_owned(*block).unwrap())
            .collect();
        let raw_old_owned: Vec<_> = destination_blocks[1..]
            .iter()
            .map(|block| allocator.is_owned(*block).unwrap())
            .collect();
        let raw_destination = inode_blocks(&mut device, &superblock, 3);
        let raw_is_old = raw_new_owned == [false, false]
            && raw_old_owned == [true, true]
