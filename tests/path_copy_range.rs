...
fn setup() -> (CrashDevice, Superblock) {
    ...
    create_symlink_journaled(&mut device, &superblock, 1, "src_alias", "/dir/src").unwrap();
    create_symlink_journaled(&mut device, &superblock, 1, "dst_alias", "/dir/dst").unwrap();
...
fn copy_cross_block(device: &mut CrashDevice, superblock: &Superblock) -> io::Result<RecoveryReport> {
...
#[test]
fn copies_existing_ranges_through_direct_and_symlink_paths() {
    ...
    assert_eq!(
        read_file_range_at_path(&mut device, &superblock, "/dir/dst", 0, BLOCK_SIZE - 2, 4).unwrap(),
...
#[test]
fn rejects_invalid_path_copy_ranges_without_metadata_publication() {
    let (mut device, superblock) = setup();
    ...
    assert_eq!(
        copy_file_range_at_path_journaled(&mut device, &superblock, "/dir", 0, 0, "/dir/dst", 0, 0, 1)
            .unwrap_err().kind(),
        io::ErrorKind::InvalidInput
    );
    assert_eq!(
        copy_file_range_at_path_journaled(&mut device, &superblock, "/dangling", 0, 0, "/dir/dst", 0, 0, 1)
            .unwrap_err().kind(),
        io::ErrorKind::NotFound
    );
    assert_eq!(
        copy_file_range_at_path_journaled(&mut device, &superblock, "/dir/src", 0, 0, "/dir/dst", 0, 0, 0)
            .unwrap_err().kind(),
        io::ErrorKind::InvalidInput
    );
    assert_eq!(
        copy_file_range_at_path_journaled(&mut device, &superblock, "/dir/src", 2, 0, "/dir/dst", 0, 0, 1)
            .unwrap_err().kind(),
        io::ErrorKind::InvalidInput
    );
...
#[test]
fn every_pathname_copy_range_crash_point_recovers_old_or_complete_new_state() {
    let (mut probe, superblock) = setup();
    probe.arm(None);
    copy_cross_block(&mut probe, &superblock).unwrap();
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(copy_cross_block(&mut device, &superblock).unwrap_err().kind(), io::ErrorKind::Other);
        device.reboot();
