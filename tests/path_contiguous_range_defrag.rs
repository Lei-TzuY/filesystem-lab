...
fn defragments_only_the_selected_fragmented_range() {
    ...
    let (new_range, report) =
        defragment_file_range_contiguous_at_path_journaled(&mut device, &superblock, "/file", 1, 2)
            .unwrap();
...
fn contiguous_selected_range_is_an_idempotent_noop() {
    ...
    let (range, report) =
        defragment_file_range_contiguous_at_path_journaled(&mut device, &superblock, "/file", 2, 2)
            .unwrap();
