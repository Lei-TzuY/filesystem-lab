...
fn assert_post_query_invariants(
    ...
    assert_eq!(
        load_inode_table(device, &superblock).unwrap().len(),
        inode_count_before + 1
    );
    assert_unique_ownership(device, &superblock);
    check_device(device).unwrap();
    assert!(load_journal_image(device, superblock).unwrap().is_empty());
    assert_eq!(
        recover_journal_and_checkpoint(device, superblock).unwrap(),
        RecoveryReport::default()
    );
}

#[test]
fn stat_recovers_committed_final_symlink_before_resolution() {
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            metadata_at_path(&mut device, &superblock, "/file_alias").unwrap(),
            PathMetadata {
                inode_id: 2,
                kind: InodeKind::File,
                logical_blocks: 0,
...
#[test]
fn lstat_recovers_committed_final_symlink_before_no_follow_lookup() {
    let (mut probe, superblock) = setup();
    probe.arm(None);
    let (link_inode, _) =
        create_symlink_journaled(&mut probe, &superblock, 1, "file_alias", "/file").unwrap();
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            symlink_metadata_at_path(&mut device, &superblock, "/file_alias").unwrap(),
            PathMetadata {
                inode_id: link_inode,
