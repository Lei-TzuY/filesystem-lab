...
fn setup() -> (CrashDevice, Superblock) {
    ...
    store_inode_table(&mut device, &superblock, &[
        ...
        inode(3, InodeKind::Directory), inode(4, InodeKind::File), inode(5, InodeKind::File),
    ]).unwrap();
    store_directory_table(&mut device, &superblock, &[
        entry(1, 2, "src"), entry(1, 3, "dst"), entry(2, 4, "source"), entry(3, 5, "target"),
    ]).unwrap();
...
fn overwrites_destination_through_parent_symlinks() {
    ...
    rename_overwrite_file_at_path_journaled(
        &mut device, &superblock, "/src_alias/source", "/dst_alias/target",
    ).unwrap();
    assert!(resolve_path_following_symlinks(&mut device, &superblock, "/src/source").is_err());
    assert_eq!(resolve_path_following_symlinks(&mut device, &superblock, "/dst/target").unwrap(), 4);
    assert!(!load_inode_table(&mut device, &superblock).unwrap().iter().any(|inode| inode.id == 5));
...
fn rejects_malformed_paths_without_publication() {
    ...
    for (source, destination) in [("src/source", "/dst/target"), ("/src/source", "dst/target"), ("/", "/dst/target"), ("/src/source", "/") ] {
        assert_eq!(rename_overwrite_file_at_path_journaled(&mut device, &superblock, source, destination).unwrap_err().kind(), io::ErrorKind::InvalidInput);
    }
...
fn every_pathname_overwrite_crash_point_recovers_old_or_complete_new_state() {
    ...
    probe.arm(None);
    rename_overwrite_file_at_path_journaled(
        &mut probe, &superblock, "/src_alias/source", "/dst_alias/target",
    ).unwrap();
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(rename_overwrite_file_at_path_journaled(
            &mut device, &superblock, "/src_alias/source", "/dst_alias/target",
