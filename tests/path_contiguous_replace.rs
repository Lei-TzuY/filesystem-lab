...
use filesystem_lab::path_contiguous_replace::replace_file_blocks_contiguous_at_path_journaled;
...
#[test]
fn replaces_an_interior_range_with_one_contiguous_run_through_a_final_symlink() {
    ...
    let (new_blocks, displaced, _) = replace_file_blocks_contiguous_at_path_journaled(
        &mut device,
        &superblock,
        "/file_alias",
        1,
    ...
    assert_eq!(new_blocks.len(), REPLACEMENT.len());
    ...
    for (offset, image) in REPLACEMENT.iter().enumerate() {
        assert_eq!(
...
#[test]
fn rejects_invalid_contiguous_replace_without_persistent_change() {
    let (mut device, superblock) = setup();
    let allocator_before = load_allocator(&mut device, &superblock).unwrap();
...
#[test]
fn every_contiguous_replace_crash_point_recovers_old_or_complete_new_state() {
    ...
    for crash_at in 0..operations {
        ...
        assert_eq!(
            ...
            .kind(),
            io::ErrorKind::Other,
            "crash point {crash_at} must interrupt contiguous replacement"
        );
        device.reboot();
